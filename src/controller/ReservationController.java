            ...
            switch (choice) {
                ...
                case 2: // Select reservation details
                    ...
                    Optional<Reservation> opt = reservationService.selectReservation(resId, currentUser.getUsername(),
                            isAdmin);
                    if (opt.isPresent())
                        handleReservationDetails(opt.get(), currentUser);
                    else
    ...
    private List<Reservation> applySortAndFilter(List<Reservation> list, String sortBy, boolean ascending,
        ...
        if (filterBy != null && filterValue != null && !filterValue.isBlank()) {
            switch (filterBy.toLowerCase()) {
                case "username" ->
                    filtered.removeIf(r -> !r.getUsername().toLowerCase().contains(filterValue.toLowerCase()));
                case "status" -> filtered.removeIf(r -> !r.getStatus().name().equalsIgnoreCase(filterValue));
                case "booktitle" ->
                    filtered.removeIf(r -> !r.getBook().getTitle().toLowerCase().contains(filterValue.toLowerCase()));
            }
        }
